...
var cur_nozzle_temp_22 = -273
echo "Current tool: " ^ state.currentTool

; ------ get_nozzle_temp(var.cur_nozzle_temp_22, state.currentTool)
set var.cur_nozzle_temp_22 = -273


var cur_tool_heater_idx_25 = 0
var cur_heater_idx_24 = -1

if state.currentTool >= 0
    ; Valid tool ID
    while var.cur_tool_heater_idx_25 < #tools[state.currentTool].heaters
        set var.cur_heater_idx_24 = tools[state.currentTool].heaters[var.cur_tool_heater_idx_25]
        
        set var.cur_nozzle_temp_22 = max(var.cur_nozzle_temp_22, heat.heaters[var.cur_heater_idx_24].current)

        set var.cur_tool_heater_idx_25 = var.cur_tool_heater_idx_25 + 1


; ------ get_nozzle_temp() END


echo "Tool temp: " ^ var.cur_nozzle_temp_22
; Allow some cooldown (just in case)
set var.can_extrude = (var.cur_nozzle_temp_22 - 5) > heat.coldExtrudeTemperature ? 1 : 0
