...
class ActionLedger:
    ...
    def copy_to_remote(self, local_path, remote_path, description):
        self.copy += (
            Action('copy', remote_path, local_path, description),
        )

    def create_remote_directory(self, remote_path):
        self.create += (
            Action('create_dir', local_path=None, remote_path=remote_path, description=''),
        )
...
PrinterFile = collections.namedtuple('PrinterFile', [
    'is_file', 'is_directory',
    'name', 'full_path', 'parent_directory',
    'date', 'size'
])
    ...
    if recursive:
        ...
        for dir_el in all_dirs:
            out += get_printer_directory(
                api, remote_path=dir_el.full_path,
                recursive=recursive
            )
    return out

LocalFile = collections.namedtuple('LocalFile', [
    'is_file', 'is_directory',
    'name', 'full_path', 'parent_directory',
...
def create_update_ledger(api, remote_dir, local_files):
    ...
    local_file_dict = dict(
        (el.printer_path, el)
        for el in local_files
    )
    remote_file_dict = dict(
        (el.full_path, el)
        for el in remote_list
    )
    ...
    for path in all_paths:
        ...
        if local_rec is None and remote_rec is not None:
            ...
            out.delete_remote(path, description=f"Deleting {path!r} as it is absent from local store")
...
def filter_ledger(ledger, create=(), copy=(), delete=()):
    ...
    for (name, rm_filters) in [
        ...
        for raw_el in raw_data:
            if not any(
                fnmatch.fnmatch(raw_el.remote_path, pattern)
                for pattern in rm_filters
            ):
...
def sync_macros(api, local_dir):
    ...
    raw_ledger = create_update_ledger(
        api,
        remote_dir='/macros', local_files=local_files,
    )
    return filter_ledger(
        raw_ledger, delete=['/macros/SETNETWORK*'])
...
def execute_ledger(printer, ledger):
    ...
    if ledger.delete:
        ...
        if yn.strip().lower() != 'y':
            ...
            return
        delete_list.extend(
            el.remote_path for el in ledger.delete
        )
